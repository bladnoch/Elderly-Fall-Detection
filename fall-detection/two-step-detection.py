...
# 세그먼트 마스크, 박스와 객체의 박스 비교
# 이동 경로를 저장해 통로, 오래 사용하는 공간 구분
# 경로들중 많이 꺾이는 구간이 있을 경우 이동이 불편하다고 판단
# 이동중 속도가 줄어드는 구간이 있을 경우 이동이 불편하다고 판단
# 한 공간에 사람 한명만 있을경우 더 민감하게 탐지
# 테이블도 안전가구로 구분
...
# 상수 정의 (fps는 이후에 설정)
MOVEMENT_THRESHOLD = 2.5  # 임계값
STATIONARY_THRESHOLD = None  # fps 기반으로 설정
...
def main():
    ...
    # 프레임률 기반으로 임계값 설정
    STATIONARY_THRESHOLD = fps * 1  # 1초 동안 정지 == 'Stationary'로 판단
